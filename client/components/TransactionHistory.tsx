    ...
    <div className="bg-primary-white rounded-[46px] p-8 h-[542px] w-[1262px] flex flex-col space-y-6">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center space-x-4">
          ...
          <ArrowUpDown size={46} className="text-primary-black cursor-pointer" />
        </div>
      </div>
        ...
        {transactions.map((transaction, index) => (
          <div key={index} className="flex justify-between items-center py-2">
            ...
            <div className="flex items-center space-x-4 w-56">
              ...
              <span className="text-primary-black text-2xl">{transaction.company}</span>
            </div>
            
            {/* Amount */}
            <div className="w-30 text-center">
              <span className="text-primary-black text-2xl">{transaction.amount}</span>
            </div>
            
            {/* Date */}
            <div className="w-44 text-center">
              <span className="text-primary-black text-2xl">{transaction.date}</span>
            </div>
            
            {/* Status */}
            <div className="w-40 flex justify-center">
              <div className={`${transaction.statusColor} rounded-[32px] px-4 py-0.5 flex items-center space-x-1`}>
                <div className={`w-1.5 h-1.5 rounded-full ${
                  transaction.status === "Paid" ? "bg-success-700" :
                  transaction.status === "Overdue" ? "bg-danger-500" :
                  "bg-warning-600"
                }`}></div>
            ...
            <div className="w-12 flex justify-center">
              <MoreHorizontal size={46} className="text-primary-black cursor-pointer" />
            </div>
          </div>
        ))}
      </div>
